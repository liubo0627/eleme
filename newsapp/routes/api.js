...
router.get('/news_cate', function(req, res) {
    console.log(req.query.cate_id)
    DB.find("news",{"cate_id":[req.query.cate_id]},function (err,data) {
        res.jsonp(data);
    })
});
...
router.post('/collect/dodelet', function(req, res) {
    ...
    DB.find('userinfo',{"_id":new ObjectId(req.body.user_id)},function (err,data) {
        if(err){
            ...
            username=data[0].username;
            console.log('uid:'+req.body.user_id)
            console.log('newid:'+req.body.news_id)
            // DB.find('news',{"_id":new ObjectId(req.body.news_id)},function (err1,data1) {
                news_id=req.body.news_id;
